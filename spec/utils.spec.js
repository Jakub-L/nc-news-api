...
describe('UTILS', () => {
  describe('jsToPsqlTimestamp()', () => {
    it('Converts single-object array to have ISO-8601 based timestamp', () => {
      ...
      expect(jsToPsqlTimestamp(input)).to.eql(expected);
    });
    ...
    it('Does not mutate input array or objects inside of it', () => {
      ...
      const output = jsToPsqlTimestamp(input);
    ...
    it('Returns converted data with specified timestamp key', () => {
      ...
      expect(jsToPsqlTimestamp(input, 'timestamp')).to.eql(expected);
    });
  });
  ...
  describe('getKeyToKeyPairing()', () => {
    it('Retrieves the key-key pair as object from single-object array', () => {
      ...
      expect(getKeyToKeyPairing(input, 'title', 'article_id')).to.eql(expected);
    });
    it('Retrieves the key-key pair as object from multiple-object array', () => {
      ...
      const expected = { 'Moby Dick': 5, 'Don Quixote': 6, 'Count of Monte Cristo': 6 };
      expect(getKeyToKeyPairing(input, 'title', 'article_id')).to.eql(expected);
    });
  });
  ...
  describe('replaceKeysOfObject()', () => {
    it('Replaces key based on lookup object in single-object array', () => {
      ...
      expect(replaceKeysOfObject(input, 'title', 'book_id', lookup)).to.eql(output);
    });
    it('Replaces key based on lookup object in multi-object array', () => {
      ...
      const lookup = { 'Moby Dick': 1, 'Winnie the Pooh': 2, 'The Old Man and the Sea': 3 };
      expect(replaceKeysOfObject(input, 'title', 'book_id', lookup)).to.eql(output);
    });
    it('Does not mutate input array or objects inside of it', () => {
      const input = [
